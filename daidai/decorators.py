...
class Asset(Depends):
    ...
    def validate(self):
        ...
        if _functions[self.fn.__name__]["type"] != ComponentType.ASSET:
            logger.error(
                f"{self.fn.__name__} is not an asset, but a {_functions[self.fn.__name__]['type'].value}"
            )
            raise TypeError(
                f"{self.fn.__name__} is not an asset, but a {_functions[self.fn.__name__]['type'].value}"
            )
...
class Predictor(Depends):
    ...
    def validate(self):
        ...
        if _functions[self.fn.__name__]["type"] != ComponentType.PREDICTOR:
            logger.error(
                f"{self.fn.__name__} is not a predictor, but a {_functions[self.fn.__name__]['type'].value}"
            )
            raise TypeError(
                f"{self.fn.__name__} is not a predictor, but a {_functions[self.fn.__name__]['type'].value}"
            )
        ...
        for param_name in sig.parameters:
            ...
            if (
                ...
                _functions[func.__name__]["dependencies"].append(
                    (param_name, dep_func, component._function_config)
                )
                ...
                _functions[func.__name__]["dependencies"].append(
                    (param_name, dep.fn, dep_defaults | dep._function_config)
                )
